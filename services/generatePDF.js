...
function generateCustomerInformation(doc, orderData) {
  ...
  doc
    .text(`Order Number: ${orderData._id}`, 50, 100)
    .text(
    ...
    .text(
      `Name: ${addressChosen.username}`,
      300,
      100
    )
    .text(
      `Address: ${addressChosen.address1} ${addressChosen.address2} `,
      300,
      115
    )
...
function generateBody(doc, orderData) {
  ...
  orderData.cartData.forEach((v, i) => {
    doc.fontSize(10).text(v.productName, 50, 260 + (i + 1) * 20);
